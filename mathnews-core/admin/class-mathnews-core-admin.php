...
class Mathnews_Core_Admin {
	...
	private function current_onboarding_value() {
		return $this->version . ';' . wp_get_current_user()->roles[0];
	}
	...
	public function enqueue_onboarding_styles() {
		...
		wp_enqueue_style( 'shepherd', plugin_dir_url( __FILE__ ) . 'css/shepherd.css', array(), '9.1.0', 'all' );
		wp_enqueue_style( $this->plugin_name . '-onboarding', plugin_dir_url( __FILE__ ) . 'css/mathnews-core-onboarding.css', array(), $this->version, 'all' );
	}
	...
	public function enqueue_onboarding_scripts() {
		...
		wp_enqueue_script( 'shepherd', plugin_dir_url( __FILE__ ) . 'js/shepherd.min.js', [], '9.1.0', true );
		wp_enqueue_script( $this->plugin_name . '-onboarding', plugin_dir_url( __FILE__ ) . 'js/mathnews-core-onboarding.js',
			['shepherd', 'jquery', $this->plugin_name], $this->version, true );
		wp_localize_script($this->plugin_name . '-onboarding', 'mn_onboarding', [
			'nonce'        => $nonce,
		]);
	}
